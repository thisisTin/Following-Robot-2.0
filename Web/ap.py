...
# --- CẤU HÌNH NGƯỠNG TRACKING (FIX LỖI XA) ---
MIN_VALID_AREA = 5000     # Nếu nhỏ hơn mức này -> Dừng, không lùi loạn xạ
TARGET_AREA = 45000       
...
def robot_logic_thread():
    ...
    # --- ĐÂY LÀ HÀM ĐÃ SỬA LỖI ĐI LÙI & MẤT TRACK KHI XA ---
    def run_pd_controller(bbox):
        ...
        # [FIX 1] Nếu quá xa (diện tích nhỏ) -> Dừng hẳn, không tính PID nữa
        if area < MIN_VALID_AREA:
            # Gửi thông báo về web để biết
            socketio.emit('telemetry', {
                'dist_front': sensor_data['front'], 
                'area_input': area, 
                'error': 0, 
                'pwm_l': 0, 'pwm_r': 0, 'pwm_fwd': 0,
                'status_msg': "TOO FAR" 
            })
            set_robot_pwm(0, 0, "TOO_FAR")
            return

        # [FIX 2] Đảo ngược công thức Error Area
        # Cũ: error = area - TARGET (Xa -> âm -> lùi) => SAI
        # Mới: error = TARGET - area (Xa -> dương -> tiến) => ĐÚNG
        ...
        set_robot_pwm(left_motor, right_motor, "PD_FOLLOW")

        # Gửi telemetry (giữ nguyên để vẽ biểu đồ)
        socketio.emit('telemetry', {
            'dist_front': sensor_data['front'],
            'area_input': area,
            'error': error_area,
            'pwm_l': int(left_motor),
            'pwm_r': int(right_motor),
            'pwm_fwd': int(fwd_speed),
            'status_msg': "TRACKING"
        })
    ...
    while True:
        ...
        if curr_state == "IDLE":
            ...
            if lost_tracking:
                ...
                # Gửi thông báo LOST để Web biết
                socketio.emit('telemetry', {
                    'dist_front': sensor_data['front'], 
                    'area_input': 0, 'error': 0, 'pwm_l': 0, 'pwm_r': 0, 'pwm_fwd': 0,
                    'status_msg': "LOST TRACKING"
                })
...
def cancel_follow():
    # --- LOGIC NGUYÊN BẢN (KHÔNG SỬA GÌ THÊM) ---
