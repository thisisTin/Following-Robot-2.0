...
def set_failsafe_distance_cm(dist_cm):
    global current_python_failsafe_cm
...
def set_robot_pwm(left_pwm, right_pwm, intent=""):
    ...
    # 1. Logic Failsafe Tại App
    with lock:
        dist_front = sensor_data['front']
        dist_back = sensor_data['back']
    
    # Nếu đang muốn TIẾN (dương/âm tùy logic motor) mà vật cản trước quá gần -> Dừng
    # Lưu ý: Nếu logic đảo chiều ở PD làm PWM âm = Tiến, thì phải sửa chỗ này.
    # Nhưng tạm thời giữ logic: PWM > 0 là kênh Tiến của mạch cầu H (nếu đấu đúng).
...
def robot_logic_thread():
    ...
    print("Logic thread started (Reversed Direction Fix)...")
    ...
    def run_pd_controller(bbox):
        ...
        # --- [SỬA LỖI NGƯỢC HƯỚNG TẠI ĐÂY] ---
        # Logic mới: area - TARGET_AREA
        # Nếu xa (area nhỏ) -> error âm -> Speed âm
        # Nếu gần (area lớn) -> error dương -> Speed dương
        # (Điều này phụ thuộc vào việc Speed âm là Tiến hay Lùi trên phần cứng của bạn)
...
def cancel_follow():
    global robot_state, target_person_id, tracker, is_searching_for_target
    with lock:
        robot_state = "IDLE"
        target_person_id = None
        tracker = None
